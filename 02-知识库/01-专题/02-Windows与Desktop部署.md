...
## 杀软墙位（v1.1 新增，c4 引用落点）

| 阶段 | 现象 | 处理 | 来源 |
|---|---|---|---|
| 安装期 | 第三方杀软拦 PortableGit/uv 载荷 | 官方文档未给安装期白名单步骤（**未验证**）；保守做法：装前临时退出第三方杀软、装完恢复，只用官网安装包 | 推断 |
| 更新期 | 杀软隔离更新器脚本→更新假失败 | 修复安装+查隔离报告后重试；**官方明令不得关杀软** | llms-full.txt:834 ✅ |
| 更新期 | 杀软 shim 制造假进程锁 | `hermes update --force`（仅此情形） | llms-full.txt:968 ✅ |

> hermes-agent GitHub README 另有 Windows Defender 误报节（精确 PowerShell 命令，2026-09-16 专题09 记载）；引用其原文前先重核仓库 README。
