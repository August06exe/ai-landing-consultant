...
- [x] **行业工作流案例库**：虚构示例矩阵开张——餐饮/财务/投资/诊所四案（`02-知识库/04-案例库/`），真实结案案例按 d5 持续回填
- [x] **工作流配方库**：六大通用模式 + 心跳配方 + 迁移 runbook（`02-知识库/05-配方库/`）
- [x] **专业领域顾问技能包**：财务/投资/数据/法务四领域（含合规红线与禁用词表）
- [ ] **平台适配层**：把交付包范式适配到 OpenClaw 等其他 agent（存量用户迁移指南）
- [ ] 11 道工序打包成 Hermes 插件/技能组（`hermes plugins install` 一条命令装顾问，参照 superpowers 形态）
- [ ] 一键部署向导：把 c4 部署引导做成给小白点「下一步」的脚本（含升级/卸载/回滚）
