...
<picture>
  ...
  <img src=".github/assets/banner-light.svg" alt="AI 落地顾问 · ai-landing-consultant" width="100%">
</picture>

# 客户的数据只住在客户电脑里

**简体中文** ｜ [English](README.en.md) ｜ 日本語（ランディング要約版）

<sub>*本仓库的核心价值（渠道坑/征期/本地案例）为中文市场语料；English 版是完整翻译，日本語版是 landing 摘要——全量翻译以贡献者招募方式进行。*</sub>
...
11 道标准工序 · 8 个顾问技能（含 4 个领域合规包） · 6 个完整示例案例 · 15 份落地专题
...
| **只要知识库** | 直接读 [`02-知识库/01-专题/`](02-知识库/01-专题/) 十五份专题与[配方库](02-知识库/05-配方库/01-六大通用模式配方.md) | 拉最新官方文档：`bash scripts/fetch-hermes-docs.sh` |

**你手上有什么，就对号入座**（反查导航）：

| 你手上有什么 | 直接看 |
|---|---|
| 微信，想要每天自动汇报 | [拉面店案例](02-知识库/04-案例库/示例-拉面店老王/README.md) · [征期提醒官](02-知识库/04-案例库/示例-征期提醒官/README.md) |
| 网店/外卖平台，被评价搞得很烦 | [餐饮评价管家](02-知识库/04-案例库/示例-餐饮评价管家/README.md) |
| 记账/对账 Excel | [财务会计月结](02-知识库/04-案例库/示例-财务会计月结/README.md) |
| 持仓/行情想每天看 | [投资人大盘追踪](02-知识库/04-案例库/示例-投资人大盘追踪/README.md) |
| 预约/客户名单管理 | [口腔诊所](02-知识库/04-案例库/示例-诊所预约管理/README.md) |

👀 **看不懂技术？10 分钟看一个完整案例**：[拉面店老王](02-知识库/04-案例库/示例-拉面店老王/README.md)——从访谈到交付包到使用卡的全过程。

## 我们的策展账目（为什么值得信）

数字全部仓库内可数，机器保镖 `scripts/check-numbers.sh` 随时可复验：

- 指令断言 **38 行判定**：25 与官方文档一致 / **11 处错误当场修源** / 2 处存疑标注（专题12）
- llms 行号锚点 **332 条机器复核**：13 处腐烂当日修复（check-anchors）
- 知识库 **15 份专题**：每条结论带来源与日期；超 3 个月强制复核
- 评审链 **5 轮**：含 1 轮无污染独立外评（6.8/10，全文存档专题14）——负面结论全部保留在案
- 案例库 **6 案全部虚构**且逐案标注——「真实」二字只留给真实交付
...
| 📚 **落地知识库 ×15** | 渠道对照与选型（含微信 iLink 的坑）、Windows/Desktop 部署、模型接入、运维排查、**密钥与隐私基线**、落地痛点调研、专业领域盘点、**Desktop 完整使用指南**、**指令核对清单**、**门面对标解剖** | 每条结论带来源与日期，指令断言逐条核对过官方文档（38 行判定：11 ✗ 当场修源） |
...
| 🔌 **可安装技能包** | 11 道工序已打包成 SKILL.md（`skills-tap/`）+ Hermes 插件薄壳（`.hermes-plugin/`） | `hermes plugins install` 或直接复制，装进任何 CLI agent |
...
02-知识库/      01-专题×15 | 02-官方文档(脚本拉取) | 03-参考技能 | 04-案例库(示例案例×6) | 05-配方库
...
## Roadmap（v2.0 重排：真实验证 > 门面打磨）

> 依据独立外评（专题14）：本仓库当前最大缺口是「外部效度」，不是文档。优先级已按此重排。

- [x] 理论钉死/门面重构/评审链建立（v1.x，2026-09-18/19）
- [x] 维护税对冲：`check-anchors.sh` 锚点机器复核（332 条断言基线）+ 平台适配层显式化 + 获客弹药（v2.0，2026-09-19）
- [ ] 🔴 **首次真实点火**：按 `docs/first-ignition.md` 完成第一单真实（或免费打样）交付，点火日志回填+案例入库——**当前最高优先级，其余一切给它让路**
- [ ] 🔴 Demo 视频置顶：60 秒「甩包→自装配→微信收汇报」录屏（分镜脚本已备：`04-运营/宣传素材/demo分镜脚本.md`）
- [ ] 11 道工序打包成 Hermes 技能组，两步走：①转 skills tap 仓库（零代码）②薄壳插件（plugin.yaml+register）——`hermes plugins install` 一条命令装顾问
- [ ] 平台适配第二例：OpenClaw（按 `02-知识库/00-平台适配层.md` 最小工作清单）
